...
class CachePlugin {
  ...
  async get (method, data) {
    ...
    // 达不到缓存条件的直接返回
    if (!this.hasCacheCharacteristics(data)) {
      return ''
    }
  ...
  save (response) {
    ...
    if (!this.isAvailable(response.data.result)) return
    let method = config.url.split('?')[0]
    let requestParams = JSON.parse(config.data || {})


    // 符合条件就缓存
    if (this.hasCacheCharacteristics(requestParams)) {
      let key = this.makeKey(method, requestParams)
      console.log('key-save', key)
      console.log('method-save', method)
      console.log('data-save', requestParams)
